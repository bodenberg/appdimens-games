...
/** FIT Strategy (Letterbox): f(x) = x × min(W/W₀, H/H₀) - Game fit, ensures content fits */
...
@Composable
fun FitDemoScreen() {
    Scaffold(topBar = { TopAppBar(title = { Text("FIT Strategy (Letterbox)") }) }) { padding ->
        LazyColumn(
            Modifier
                .fillMaxSize()
                .padding(padding)
            ...
            item {
                InfoCard(
                    "📖 FIT (Letterbox)",
                    "Game fit - ensures content fits"
                ) {
                    Text("Formula: f(x) = x × min(W/W₀, H/H₀)"); Text("Uses smaller ratio"); Text("Ensures everything fits (may leave space)"); Text(
                    "Best for: Games, full-screen content"
                )
                }
            }
            item {
            ...
            item {
                InfoCard(
                    ...
                    Text("✅ Game letterbox mode"); Text("✅ Content that must fit"); Text("✅ Full-screen layouts"); Text(
                    "⚠️ May leave empty space"
                )
                }
            }
        }
